...
// Traductions à ajouter pour chaque langue
...
// Patterns pour trouver la fin de la section login pour chaque langue
const patterns = {
  es: "        errorMessage: 'Código inválido. Por favor intente de nuevo.',\n      },\n    },",
  pt: "        errorMessage: 'O código que você digitou é inválido. Tente novamente.',\n      },\n    },",
  it: "        errorMessage: 'Il codice inserito non è valido o è scaduto. Riprova.',\n      },\n    },",
  de: "        errorMessage: 'Der eingegebene Code ist ungültig oder abgelaufen. Versuchen Sie es erneut.',\n      },\n    },",
  nl: "        errorMessage: 'De ingevoerde code is ongeldig of verlopen. Probeer opnieuw.',\n      },\n    },"
};
...
async function patchTranslations() {
  ...
  // Pour chaque langue, remplacer le pattern
  for (const [lang, pattern] of Object.entries(patterns)) {
    console.log(`🔍 Recherche de ${lang.toUpperCase()}...`);
    
    const replacement = pattern.replace('    },', `    },\n${adminSettingsSections[lang]}`);
    ...
    if (content.includes(pattern)) {
      ...
      console.log(`⚠️  Pattern non trouvé pour ${lang.toUpperCase()}`);
    }
  }
  
  if (patchedCount > 0) {
    console.log(`💾 Écriture du fichier modifié (${patchedCount} langues corrigées)...`);
    fs.writeFileSync(filePath, content, 'utf-8');
    console.log('✨ Traductions ajoutées avec succès!');
  } else {
    console.log('⚠️  Aucune traduction ajoutée. Les patterns ne correspondent pas.');
  }
}
